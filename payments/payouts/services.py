...
#     def __init__(self, user, amount_kobo, role=None):
#         self.user = user
...
#         return UserAccount.objects.filter(user=self.user).first()

#     def check_cooldown(self):
#         last_complete = Withdrawal.objects.filter(
#             user=self.user, status="complete"
#         ).order_by("-completed_at").first()

#         if not last_complete:
#             return True

#         return (self.now - last_complete.completed_at) >= timedelta(
#             hours=WITHDRAWAL_COOLDOWN_HOURS
#         )
